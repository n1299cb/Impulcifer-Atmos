...
class RealTimeConvolver:
    ...
    def __init__(self, hrir, block_size: int = 1024):
        self.block_size = block_size
        self.fs = hrir.fs
        self.speakers = list(hrir.irs.keys())
        self.n_speakers = len(self.speakers)
        self._prepare_ir_fft(hrir)
        self.overlap = np.zeros((2, self.fft_size - self.block_size))

    def _next_pow2(self, x: int) -> int:
        return 1 << (x - 1).bit_length()

    def _prepare_ir_fft(self, hrir) -> None:
        max_len = 0
...
if __name__ == "__main__":
    ...
    parser.add_argument("hrir", help="hrir.wav generated by Impulcifer")
