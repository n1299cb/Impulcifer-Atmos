...
class ImpulseResponseEstimator(object):
    ...
    def sweep_sequence(self, speakers, tracks):
        ...
        if tracks == "9.1.6":
            ...
            raise ValueError(
                "Unsupported track configuration '{}'".format(tracks)
            )
...
def create_cli():
    ...
    arg_parser.add_argument(
        ...
        default="FL",
        help=(
            "Speaker channel order in test signal sequence as comma separated values. "
            "Mono test signal sequence is created by default but if speakers are "
            "specified a second sequence file will be generated which contains multiple "
            "sweeps separated by silences in the order of given speaker channels. "
            'Stereo sequence can be generated by supplying value "FL,FR". Supported '
            'channel names are "FL", "FR", "FC", "SL", "SR", "BL" and "BR".'
        ),
    )
    arg_parser.add_argument(
        "--tracks",
        ...
        default="mono",
        help=(
            'WAV file track configuration. Supported values are "mono", "stereo", "5.1", '
