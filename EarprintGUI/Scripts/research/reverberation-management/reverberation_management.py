...
def main():
    ...
    parser.add_argument(
        '--track_order', type=str, required=True, help='Track order in HRIR file. "hesuvi" or "hexadecagonal"'
    )
    parser.add_argument(
        '--reverb',
        type=str,
    ...
    for ch, t in reverb.items():
        ...
        win = (
            np.concatenate(
                [np.ones(n_ones), signal.windows.hann(n_win * 2)[n_win:], np.zeros(data.shape[1] - n_ones - n_win)]
            )
            - 1.0
        )
        win *= 100  # 100 dB
